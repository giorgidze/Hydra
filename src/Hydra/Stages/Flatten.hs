...
flatten symtab = go (symtab{equations=[],variables = Map.empty}) (flattenEqs 0 (model symtab))
  ...
  go acc ((Equal e1 e2) : eqs)      = let e3 = simplify e1
                                      ...
                                      in  go (acc1 {equations = (PrimApp Sub (Pair e3 e4)) : (equations acc1)}) eqs
  ...
  go acc ((Init (PrimApp Mul (Pair (Const (-1)) e1)) e2) : eqs) = go acc ((Init e1 (PrimApp Neg (simplify e2))) : eqs)
...
simplify e = case e of
  ...
  Var   _ -> e
  PrimApp Der (Var _) -> e
  PrimApp Der (PrimApp Mul (Pair (Const (-1)) e1)) -> PrimApp Neg (simplify (PrimApp Der e1))
  PrimApp Der _ -> error "This version of Hydra only supports first order derivatives."
